...
def input_product_id(prompt):
    while True:
        ...
        print('Invalid Product ID: ' + id_str)
...
def remove_product():
    ...
    id_to_remove = input_product_id('Enter a Product ID to remove: ')
    features_to_remove = None
    for product_features in store_inventory:
        if product_features['product_id'] == id_to_remove:
            features_to_remove = product_features

    if features_to_remove is None:
        print('Product ID ' + str(id_to_remove) + ' not found.')
    else:
        store_inventory.remove(features_to_remove)
